...
<div className="w-[1440px] h-[1000px] px-10 relative left-1/2 -translate-x-1/2 bg-Color-3 inline-flex flex-col justify-start items-center overflow-hidden">
  ...
  <div className="w-[623px] h-[621px] left-[120px] top-[280px] absolute bg-[#AF2E38] rounded-[20px] outline outline-1 outline-offset-[-1px] outline-[#AF2E38] overflow-y-auto overflow-x-hidden scrollbar-hide scroll-smooth py-6 px-4">
    <div className="grid grid-cols-2 justify-items-center gap-x-2 gap-y-3 w-full">
      {products.map((flower) => (
         <div key={flower._id} className="relative w-[286px] h-[360px] flex flex-col items-center" style={{ overflow: 'hidden' }}>
            {/* Background base */}
            <img className="absolute top-0 w-[286px] h-[360px] drop-shadow-md z-0" src="/images/CustomizeHoa/nenhoa.png" alt="nen" />
            
            {/* Inner Content */}
            <div className="absolute inset-0 flex flex-col z-10 pointer-events-none" style={{ padding: '20px 20px 16px 20px', gap: '8px', overflow: 'hidden' }}>
               {/* Price */}
               <div className="pointer-events-auto" style={{ fontSize: '14px', fontWeight: '700', fontStyle: 'italic', color: '#AF2E38', paddingLeft: '10px' }}>
                 {new Intl.NumberFormat('vi-VN').format(flower.price)} VNĐ/cành
               </div>
               
               {/* Image */}
               <div className="flex justify-center items-center pointer-events-auto" style={{ height: '150px' }}>
                 <img className="object-contain hover:scale-105 transition" style={{ height: '145px', width: 'auto' }} src={flower.imageUrl} alt={flower.name} />
               </div>
               
               {/* Name */}
               <div className="text-center pointer-events-auto" style={{ color: '#3B73A9', fontSize: '18px', fontWeight: '700', fontFamily: 'Geologica', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                 {flower.name}
               </div>
               
               {/* Description */}
               <div style={{ color: '#444', fontSize: '13px', fontStyle: 'italic', fontFamily: 'Geologica', lineHeight: '1.3', wordBreak: 'break-word', overflowWrap: 'break-word', overflow: 'hidden', textAlign: 'center', maxHeight: '48px', width: '100%', boxSizing: 'border-box', padding: '0 12px' }}>
                 {flower.description?.substring(0, 25)}{flower.description?.length > 25 ? '...' : ''}
               </div>

               {/* Buttons */}
               <div className="flex items-center justify-center gap-4 pointer-events-auto">
                 <button onClick={() => decrease(flower._id)} style={{ background: 'transparent', border: 'none', color: '#AF2E38', fontSize: '24px', fontWeight: '500', width: '34px', height: '34px', padding: 0, cursor: 'pointer' }}>-</button>
                 <span style={{ color: '#AF2E38', fontSize: '17px', fontWeight: '700', fontStyle: 'italic', width: '20px', textAlign: 'center' }}>{counts[flower._id] || 0}</span>
                 <button onClick={() => increase(flower._id)} style={{ background: 'transparent', border: 'none', color: '#AF2E38', fontSize: '24px', fontWeight: '500', width: '34px', height: '34px', padding: 0, cursor: 'pointer' }}>+</button>
               </div>
            </div>
         </div>
      ))}
