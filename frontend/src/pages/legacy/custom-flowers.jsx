...
<div className="w-[1440px] h-[1000px] px-10 relative left-1/2 -translate-x-1/2 bg-Color-3 inline-flex flex-col justify-start items-center overflow-hidden">
  ...
  <div className="w-[623px] h-[621px] left-[120px] top-[280px] absolute bg-[#AF2E38] rounded-[20px] outline outline-1 outline-offset-[-1px] outline-[#AF2E38] overflow-y-auto overflow-x-hidden scrollbar-hide scroll-smooth py-6 px-5">
    <div className="grid grid-cols-3 justify-items-center gap-x-4 gap-y-5 w-full">
      {products.map((flower) => (
         <div key={flower._id} className="relative w-[180px] h-[300px] flex flex-col items-center" style={{ overflow: 'hidden' }}>
            {/* Background base */}
            <img className="absolute top-0 w-[180px] h-[300px] drop-shadow-md z-0" src="/images/CustomizeHoa/nenhoa.png" alt="nen" />
            
            {/* Inner Content */}
            <div className="absolute inset-0 flex flex-col z-10 pointer-events-none" style={{ padding: '14px 12px 12px 12px', gap: '6px', overflow: 'hidden' }}>
               {/* Price */}
               <div className="pointer-events-auto" style={{ fontSize: '12px', fontWeight: '700', fontStyle: 'italic', color: '#AF2E38', paddingLeft: '6px' }}>
                 {new Intl.NumberFormat('vi-VN').format(flower.price)} VNĐ/cành
               </div>
               
               {/* Image */}
               <div className="flex justify-center items-center pointer-events-auto" style={{ height: '116px' }}>
                 <img className="object-contain hover:scale-105 transition" style={{ height: '112px', width: 'auto' }} src={flower.imageUrl} alt={flower.name} />
               </div>
               ...
               {/* Description */}
               <div style={{ color: '#444', fontSize: '11px', fontStyle: 'italic', fontFamily: 'Geologica', lineHeight: '1.25', wordBreak: 'break-word', overflowWrap: 'break-word', overflow: 'hidden', textAlign: 'center', maxHeight: '34px', width: '100%', boxSizing: 'border-box', padding: '0 8px' }}>
                 {flower.description?.substring(0, 25)}{flower.description?.length > 25 ? '...' : ''}
               </div>

               {/* Buttons */}
               <div className="flex items-center justify-center gap-3 pointer-events-auto">
                 <button onClick={() => decrease(flower._id)} style={{ background: 'transparent', border: 'none', color: '#AF2E38', fontSize: '22px', fontWeight: '500', width: '28px', height: '28px', padding: 0, cursor: 'pointer' }}>-</button>
                 <span style={{ color: '#AF2E38', fontSize: '15px', fontWeight: '700', fontStyle: 'italic', width: '18px', textAlign: 'center' }}>{counts[flower._id] || 0}</span>
                 <button onClick={() => increase(flower._id)} style={{ background: 'transparent', border: 'none', color: '#AF2E38', fontSize: '22px', fontWeight: '500', width: '28px', height: '28px', padding: 0, cursor: 'pointer' }}>+</button>
               </div>
            </div>
         </div>
      ))}
